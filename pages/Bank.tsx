...
const Bank: React.FC = () => {
  ...
  const transactions = [
    { id: 'TX-9012', desc: 'Monthly Payout: June 2024', date: 'Jul 01, 2024', amount: '+$7,400.00', status: 'Completed' },
    { id: 'TX-8841', desc: 'Bonus: Advanced Calculus Course Performance', date: 'Jun 20, 2024', amount: '+$500.00', status: 'Completed' },
    { id: 'TX-8722', desc: 'Monthly Payout: May 2024', date: 'Jun 01, 2024', amount: '+$5,900.00', status: 'Completed' },
    { id: 'TX-8601', desc: 'Exam Proctoring Fee: Bio-Genetics', date: 'May 15, 2024', amount: '+$120.00', status: 'Completed' },
  ];
  ...
  return (
    <div className="p-6 lg:p-10 space-y-12 animate-fade-in max-w-[1600px] mx-auto pb-24 lg:pb-10 bg-slate-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          { label: 'Total Revenue', value: '$84,240', icon: 'payments', color: 'brand' },
          { label: 'Available Balance', value: '$12,400', icon: 'account_balance', color: 'emerald' },
          { label: 'Pending Payouts', value: '$2,150', icon: 'hourglass_empty', color: 'amber' },
          { label: 'YTD Growth', value: '+18.4%', icon: 'trending_up', color: 'indigo' }
        ].map((s, i) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        ...
        <div className="bg-slate-900 rounded-[32px] p-12 text-white shadow-2xl flex flex-col min-h-[500px]">
          <h3 className="text-xl font-black tracking-widest uppercase mb-10">Payout History</h3>
            ...
            {transactions.map((tx, i) => (
              <div key={i} className="flex flex-col gap-2 p-6 bg-white/5 rounded-2xl border border-white/5 group hover:bg-white/10 transition-all cursor-pointer">
                <div className="flex justify-between items-start">
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{tx.id}</span>
                  <span className="text-xs font-black text-brand-400">{tx.amount}</span>
                </div>
                <h4 className="text-sm font-bold tracking-tight text-white">{tx.desc}</h4>
                <div className="flex justify-between items-center mt-2">
                   <span className="text-[9px] font-black text-slate-400 uppercase">{tx.date}</span>
                   <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 rounded text-[8px] font-black uppercase tracking-widest">Done</span>
                </div>
              </div>
            ))}
          </div>
